...
import { distanceMetres, STAGES } from "../telemetry.js";
...
export function MapView({ active }) {
  ...
  const { point, trackPoints, prediction, groundStations, userLocation, velocity, status } = useTracker();
  useEffect(() => {
  ...
  const range = point && userLocation ? (distanceMetres(userLocation, point) / 1000).toFixed(2) : "—";
  return <div className="map-view">
    <div ref={target} className="map" aria-label="Rocket tracking map" />
    <aside className="telemetry-card glass" aria-live="polite">
      <Metric label="Altitude" value={point?.alt?.toFixed(1)} unit="m" hero />
      <Metric label="Vertical speed" value={point ? velocity.vertical.toFixed(1) : "—"} unit="m/s" />
      <Metric label="Range" value={range} unit="km" />
      <details><summary>More telemetry</summary><div className="detail-metrics"><Metric label="Signal" value={point?.rssi} unit="dBm" /><Metric label="Satellites" value={point?.sats} /><Metric label="Packets" value={trackPoints.length} /></div></details>
    </aside>
    <div className={`stage-badge stage-${point?.stage ?? "waiting"}`}><i />{point ? STAGES.get(point.stage) || "UNKNOWN" : status.connected ? "WAITING FOR TELEMETRY" : "BOARD OFFLINE"}</div>
  </div>;
}
